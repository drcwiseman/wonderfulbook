...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/verify-email/:token', async (req, res) => {
    try {
      ...
      res.status(500).send(`
        <html>
          <head>
            <title>Verification Error - Wonderful Books</title>
            <style>
  ...
  app.post("/api/pdf-token/:bookId", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!canAccess) {
        return res.status(403).json({ 
          message: `This book requires ${book.requiredTier} subscription`,
      ...
      res.status(500).json({ message: "Failed to generate PDF access token" });
    }
  });
